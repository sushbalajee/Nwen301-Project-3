...
//----------------------------------------------------------------------
  ...
  tid = thread_create (file_copy, PRI_DEFAULT, start_process, &exec);
...
//------------------------------ My Code ------------------------------
  ...
  if(tid!=TID_ERROR){
  	...
  	//Wait for the new user program to be loaded successfully by start_process.
    sema_down(&exec.load_done);
    
    //check whether the execution status is successful.
    if(exec.success){
...
//------------------------------ My Code ------------------------------
  ...
  //If loading is successful, allocate kernel memory space to hold the wait_status, using malloc().
  if (success){
    ...
    //Properly initialize the struct wait_status.
...
//------------------------------ My Code ------------------------------
  ...
  struct list_elem *child_elem = list_begin(&curr->children);
  
  //Go through each child one by one.
  while(child_elem!=list_end(&curr->children)){

    struct wait_status *cs = list_entry(child_elem, struct wait_status, elem);
...
{
  struct thread *t = thread_current ();
  char arguments[NAME_MAX +2];
  char file_name[NAME_MAX + 2];
  struct Elf32_Ehdr ehdr;
  struct file *file = NULL;
  off_t file_ofs;
  bool success = false;
  char *cp, *save_ptr, *f_name;
  int i;
  ...
  while (*cmd_line == ' ')
    cmd_line++;
  strlcpy (arguments, cmd_line, sizeof arguments);
  
  f_name = strtok_r(arguments," ",&save_ptr);
  strlcpy (file_name, f_name, sizeof(file_name));
...
//------------------------------ My Code ------------------------------
   ...
   size_t padded_size = ROUND_UP(size, sizeof (uint32_t));
    ...
    *ofs-=padded_size;
    ...
    memcpy(kpage+*ofs+(padded_size-size),buf,size);
 
    return kpage+*ofs+(padded_size-size);
...
//------------------------------ My Code ------------------------------
    ...
    char *cmd_copy;
    char *kernel_arg, *save_ptr;
    int argc;
    char **argv;

    cmd_copy = push(kpage, &ofset, cmd_line, strlen(cmd_line) + 1);

    if(push(kpage, &ofset, &null, sizeof null) == NULL){
        return false;
    }

    argc = 0;

    for(kernel_arg = strtok_r (cmd_copy," ",&save_ptr); kernel_arg !=NULL; kernel_arg = strtok_r(NULL," ",&save_ptr)){
        void *user_arg = upage + (kernel_arg - (char*) kpage);
       
        if(push(kpage, &ofset, &user_arg, sizeof user_arg) == NULL){
           return false;
        }
    ...
    argv = (char**)(upage+ofset);
    reverse(argc,(char**)(kpage+ofset));


 if (push (kpage, &ofset, &argv, sizeof argv) == NULL || push (kpage, &ofset, &argc, sizeof argc) == NULL || push (kpage, &ofset, &null, sizeof null) == NULL){
    return false;
    }
